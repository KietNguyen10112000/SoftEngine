...
float4 NormalShading(PS_INPUT input)
{
	...
	curPixel.normal = normalize(input.normal);
...
float4 NormalMapShading(PS_INPUT input)
{
	...
	norm = norm * 2 - 1;
	curPixel.normal = normalize(norm + curPixel.normal);

	curPixel.shininess = shininess;

	curPixel.depth = input.pos.z / input.pos.w;
...
float4 PBRShading(PS_INPUT input)
{
	...
	pixel.depth = input.pos.z / input.pos.w;
