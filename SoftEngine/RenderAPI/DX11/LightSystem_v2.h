...
protected:
	...
	void UpdateDirLight(ExtraDataDirLight* p, Light& light, Mat4x4* vpMat, float near, float far, float w, float h);
...
public:
	...
	void AddShadow(LightID id, Vec2 shadowMapDimentions, float near = 0.5f, float far = 250.0f, float w = 100, float h = 100);
	void AddShadow(LightID id, SHADOW_MAP_QUALITY quality, float near = 0.5f, float far = 250.0f, float w = 100, float h = 100);
	...
	void UpdateShadow(LightID id, float near = -1, float far = -1, float w = -1, float h = -1);
